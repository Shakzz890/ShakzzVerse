...
import com.shakzz.verse.entities.SearchData
...
class HotStarMirrorProvider : MainAPI() {
    ...
    // Fixed Syntax Error: List<<SearchResponse> -> List<SearchResponse>
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        // Fixed Syntax Error & Null Safety: parsed<<SearchData> -> parsedSafe<SearchData>
        val data = app.get(url, referer = "$mainUrl/home", cookies = cookies).parsedSafe<SearchData>() ?: return emptyList()

        return data.searchResult?.mapNotNull {
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        // Fixed Syntax Error: parseJson<<Id> -> parseJson<Id>
        ...
        // Fixed Syntax Error & Null Safety
        val data = app.get(
        ...
        // Fixed Crash: first() -> firstOrNull()
        if (data.episodes?.firstOrNull() == null) {
            // Fixed Crash: Added .toJson()
            ...
            if (data.nextPageShow == 1 && data.nextPageSeason != null) {
                // BUG FIX: Passed 'id' instead of 'url' (which is JSON)
                episodes.addAll(getEpisodes(title, id, data.nextPageSeason, 2))
            }

            // Fixed Concurrency: Flatten amap safely
            val extraEpisodes = data.season?.dropLast(1)?.amap {
                getEpisodes(title, id, it.id, 1) // BUG FIX: Passed 'id' instead of 'url'
            }?.flatten() ?: emptyList()
    ...
    // Fixed Syntax Error: List<<Episode> -> List<Episode>
    ...
    override suspend fun loadLinks(
        ...
        // Fixed Syntax Error: parseJson<<LoadData>
