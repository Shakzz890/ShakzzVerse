...
// Added a fallback User-Agent since the old Cloudstream import was broken
...
suspend fun resolveApiUrl(): String {
    ...
    for (encoded in newTvDomains) {
        ...
        try {
            // Fixed Syntax Error: parsed<NewTvTokenResponse>()
            val response = app.get("$base/checknewtv.php", headers = newTvBaseHeaders)
                .parsedSafe<NewTvTokenResponse>()
            val tokenHash = response?.token_hash

            if (!tokenHash.isNullOrBlank()) {
                // Asserted non-null (!!) to fix the Argument Type Mismatch
