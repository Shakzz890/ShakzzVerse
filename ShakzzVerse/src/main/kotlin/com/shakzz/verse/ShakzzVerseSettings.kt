...
import com.lagradost.cloudstream3.plugins.Plugin // Assuming your plugin extends this

data class StudioOption(val key: String, val label: String, val cookieValue: String)

class ShakzzVerseSettings(
    private val plugin: Plugin, // Adjusted to standard Plugin class to prevent unresolved reference
    private val sharedPref: SharedPreferences?,
    private val studios: List<StudioOption> // Fixed the << syntax error
    ...
    private fun getDrawable(name: String): Drawable? {
        val id = plugin.resources?.getIdentifier(name, "drawable", "com.shakzz.verse")
    ...
    private fun getString(name: String): String? {
        val id = plugin.resources?.getIdentifier(name, "string", "com.shakzz.verse")
        return id?.let { plugin.resources?.getString(it) }
    }

    @SuppressLint("DiscouragedApi")
    private fun <T : View> View.findViewByName(name: String): T? {
        val id = plugin.resources?.getIdentifier(name, "id", "com.shakzz.verse")
    ...
    override fun onCreateView(
        ...
        val layoutId = plugin.resources?.getIdentifier("settings", "layout", "com.shakzz.verse")
        return layoutId?.let {
    ...
    private fun getStudioRow(option: StudioOption): RelativeLayout {
        val relativeLayout = RelativeLayout(this@ShakzzVerseSettings.requireContext()).apply {
        ...
        val checkBox = CheckBox(this@ShakzzVerseSettings.requireContext()).apply {
        ...
        val textView = TextView(this@ShakzzVerseSettings.requireContext()).apply {
            id = View.generateViewId()
            text = option.label
            textSize = 16f
            layoutParams = RelativeLayout.LayoutParams(
